...
const RoomChatPanel = ({ room, currentUserId }) => {
  ...
  const reactionCooldown = useRef(new Set());
  ...
  useEffect(() => {
    ...
    if (hasNewMessages && newMessagesCount > 0) {
      ...
      const hasOwnMessage = latestMessages.some(
      ...
      const searchRequest = {
        ...
        messageType:
          filters.messageType !== "all"
            ? filters.messageType.toUpperCase()
            : null,
        page: 0,
        size: 100,
      };
  ...
  // Only the handleSendMessage function from RoomChatPanel.js
  // Replace the existing handleSendMessage function with this fixed version

  const handleSendMessage = (content) => {
    console.log("🎯 RoomChatPanel: handleSendMessage called with:", {
      content,
    ...
    // Handle both string content and reply objects properly
    let messageToSend;

    if (replyingTo) {
      // For replies, always create an object structure
      ...
      console.log("🎯 RoomChatPanel: Created reply message:", messageToSend);
      setReplyingTo(null); // Clear reply state
    } else {
      // For regular messages, pass through as-is
      messageToSend = content;
      console.log("🎯 RoomChatPanel: Regular message:", messageToSend);
    }
...
const handleReactToMessage = async (messageId, reactionType) => {
  const reactionKey = `${messageId}_${reactionType}`;
  
  // FIXED: Prevent duplicate reactions with cooldown
  if (reactionCooldown.current.has(reactionKey)) {
    console.log('🎯 RoomChatPanel: Reaction in cooldown, ignoring:', reactionKey);
    return;
  }
  
  try {
    reactionCooldown.current.add(reactionKey);
    
    console.log('🎯 RoomChatPanel: Toggling reaction:', { 
      messageId, 
      reactionType,
      timestamp: new Date().toISOString()
    });

    // FIXED: Always use toggle endpoint - backend handles add/remove logic
    const response = await chatAPI.toggleReaction(messageId, reactionType);
    
    console.log('🎯 RoomChatPanel: Reaction toggled successfully:', response.data);
    
    // Remove from cooldown after success
    setTimeout(() => {
      reactionCooldown.current.delete(reactionKey);
    }, 1000);
    
    // Don't show success toast as reactions update in real-time via WebSocket
    
  } catch (error) {
    console.error('🎯 RoomChatPanel: Failed to handle reaction:', error);
    
    // Remove from cooldown immediately on error
    reactionCooldown.current.delete(reactionKey);
  ...
  const getRoomTitle = () => {
    if (room?.roomType === "DIRECT_MESSAGE") {
      return (
        room.dmParticipantName || room.dmParticipantUsername || "Direct Message"
      );
    }
      ...
      return (
        <div
          className={`${size} rounded-full ${
            colors[room.dmParticipantStatus] || colors.offline
          }`}
        />
      );
    }
  ...
  if (!room) {
    return (
      <div className="flex-1 flex items-center justify-center bg-gray-50">
        <div
          className={`text-center max-w-md mx-auto ${isMobile ? "p-3" : "p-6"}`}
        >
          <div
          ...
          >
            <MessageCircle
              className={`text-gray-400 ${isMobile ? "h-6 w-6" : "h-10 w-10"}`}
            />
          </div>
          <h3
  ...
  return (
    <div className="flex-1 flex flex-col bg-white relative overflow-hidden">
        ...
        <div className="flex items-center justify-between">
          ...
          >
            <div className="flex-shrink-0 relative">
              <div
                className={`text-gray-600 bg-gray-100 rounded-lg ${
                  isMobile ? "p-1.5" : "p-2"
                }`}
              >
                {getRoomIcon()}
                  ...
                  <div className="py-1">
                    ...
                    {room?.roomType === "GROUP_CHAT" && (
                      <button
                        ...
                        className={`flex items-center w-full text-gray-700 hover:bg-gray-100 ${
                          isMobile
                            ? "px-3 py-2 text-xs sm:hidden"
                            : "px-4 py-2 text-sm sm:hidden"
                        }`}
                      >
                        <Users
                          className={`mr-2 ${
              ...
              <div className="flex-1 relative">
                ...
                <input
                  ...
                  className={`w-full border border-gray-300 rounded-lg focus:outline-none focus:ring-2 focus:ring-blue-500 focus:border-transparent ${
                    isMobile ? "pl-8 pr-3 py-2 text-sm" : "pl-10 pr-4 py-2.5"
                  }`}
                />
                {searchLoading && (
                  <div
      ...
      >
        {hasMoreMessages && <div ref={topObserverRef} className="h-1" />}
        ...
        {!hasMoreMessages && messages.length > 20 && (
          <div className={`flex justify-center ${isMobile ? "py-2" : "py-4"}`}>
            <span
              className={`text-gray-500 bg-white px-3 py-1 rounded-full shadow-sm ${
                isMobile ? "text-xs" : "text-sm"
              }`}
            >
        ...
        {error && (
          <div
            className={`mx-4 my-4 p-3 bg-red-50 border border-red-200 rounded-lg text-red-700 ${
              isMobile ? "text-xs" : "text-sm"
            }`}
          >
            {error}
        ...
        {loading && messages.length === 0 ? (
          ...
          <div
            className={`flex-1 flex flex-col items-center justify-center text-gray-500 p-3 ${
              isMobile ? "min-h-40" : "min-h-64 p-6"
            }`}
          >
            <div
              className={`bg-gray-100 rounded-full flex items-center justify-center mb-4 ${
              ...
              return (
                <div
                  key={`message-${message.id}-${
                    message.reactions || "no-reactions"
                  }-${index}`}
                  id={`message-${message.id}`}
                  className={`transition-colors duration-300 ${
                    searchResults.some((result) => result.id === message.id)
                      ? "bg-yellow-50"
                      : ""
                  }`}
                >
      ...
      {showScrollToBottom && !showNewMessageAlert && (
        ...
        >
          <ArrowDown
            className={`mx-auto ${isMobile ? "h-4 w-4" : "h-5 w-5"}`}
          />
        </button>
      )}
      ...
      {showRoomInfo && (
        <div
          className={`fixed inset-y-0 right-0 bg-white border-l border-gray-200 shadow-xl z-50 overflow-y-auto ${
            ...
            <div className={isMobile ? "space-y-4" : "space-y-6"}>
              <div>
                ...
                {room?.roomDescription && (
                  <p
                    className={`text-gray-600 p-3 bg-gray-50 rounded-lg ${
              ...
              <div className={isMobile ? "space-y-1" : "space-y-2"}>
                ...
                >
                  <Search
                    className={`mr-2 ${isMobile ? "h-3 w-3" : "h-4 w-4 mr-3"}`}
                  />
