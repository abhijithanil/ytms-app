...
class WebSocketService {
  ...
  subscribe(destination, id, handler) {
    if (!this.connected || !this.client) {
      console.warn(
        `🔌 WebSocketService: Cannot subscribe to ${destination} - not connected`
      );
      return false;
    }
    ...
    try {
      console.log(
        `🔌 WebSocketService: Subscribing to ${destination} with id ${id}`
      );
      ...
      const subscription = this.client.subscribe(destination, (message) => {
        try {
          ...
          console.log(
            `🔌 WebSocketService: Received message on ${destination}:`,
            parsedMessage
          );
          if (handler) {
      ...
      console.log(
        `🔌 WebSocketService: Successfully subscribed to ${destination}`
      );
      return true;
    } catch (error) {
      console.error(
        `🔌 WebSocketService: Error subscribing to ${destination}:`,
        error
      );
      return false;
    }
  ...
  unsubscribe(destination) {
    ...
    if (subscriptionKey && this.subscriptions.has(subscriptionKey)) {
      ...
      try {
        ...
        console.log(
          `🔌 WebSocketService: Successfully unsubscribed from ${destination}`
        );
        return true;
      } catch (error) {
        console.error(
      ...
      console.warn(
        `🔌 WebSocketService: No subscription found for ${destination}`
      );
      return false;
    }
  }
  ...
  clearSubscriptions() {
    ...
    for (const [key, subscription] of this.subscriptions.entries()) {
      try {
        ...
        console.error(
          `🔌 WebSocketService: Error unsubscribing ${key}:`,
          error
        );
      }
    }
  ...
  sendMessage(destination, payload) {
    if (!this.connected || !this.client) {
      console.warn(
        `🔌 WebSocketService: Cannot send message to ${destination} - not connected`
      );
    ...
    try {
      console.log(
        `🔌 WebSocketService: Sending message to ${destination}:`,
        payload
      );
      this.client.publish({
        destination: destination,
      ...
      console.error(
        `🔌 WebSocketService: Error sending message to ${destination}:`,
        error
      );
      return false;
    }
  ...
  processMessageQueue() {
    console.log(
      `🔌 WebSocketService: Processing ${this.messageQueue.length} queued messages`
    );
  ...
  subscribeToRoom(roomId) {
    ...
    console.log(
      `🔌 WebSocketService: Subscribing to room ${roomId} with enhanced features`
    );
  ...
  sendChatMessage(content, taskId = null) {
    console.log("🔌 WebSocketService: Sending chat message:", {
      content,
      taskId,
    });
    const destination = taskId
      ? `/app/chat/task/${taskId}`
      : "/app/chat/global";
    return this.sendMessage(destination, { content: content });
  }

  sendTypingIndicator(isTyping, taskId = null) {
    console.log("🔌 WebSocketService: Sending typing indicator:", {
      isTyping,
      taskId,
    });
    const destination = taskId
      ? `/app/typing/task/${taskId}`
      : "/app/typing/global";
  ...
  sendRoomMessage(roomId, payload) {
    console.log("🔌 WebSocketService: sendRoomMessage called with:", {
      roomId,
      payload,
    ...
    console.log("🔌 WebSocketService: Enhanced payload:", enhancedPayload);

    return this.sendMessage(`/app/chat/room/${roomId}`, enhancedPayload);
  }
  ...
  sendRoomTypingIndicator(roomId, isTyping) {
    console.log(
      `🔌 WebSocketService: Sending room typing indicator for ${roomId}:`,
      isTyping
    );
    return this.sendMessage(`/app/typing/room/${roomId}`, {
      isTyping: isTyping,
    });
  }
  ...
  subscribeToMentions(userId) {
    return this.subscribe(
      `/user/${userId}/queue/mentions`,
      `userMentions_${userId}`,
      (mention) => {
  ...
  getEnhancedConnectionState() {
    return {
      ...
      connectionUptime:
        this.connected && this.connectionStartTime
